...
//Test for delite link
  ...
  $job->save();
   //Call method is from GET or POST and read the url
  $browser->
    call(sprintf('/en/job/%s/extend', $job->getToken()), 'put', array('_with_csrf' => true))->
    with('response')->isRedirected()
  ;
   //After updating the job data by action, need to reload the local object
  $job->refresh();

  //Test the new active period using the embedded lime object directly
  $browser->test()->is(
  ...
  //Simulate job posting in token field
  $browser->
    get('/en/job/new')->
    click('Preview your job', array('job'=> array(
      'token'=> 'fake_token',
    ...
    with('form')->begin()->
      hasErrors(7)->
      hasGlobalError('extra_fields')-> //default form does not allow the values submitted into additional field
    end()
  ;
...
//Day 13 User Test
  $browser->
    ...
    //In order to make life easier, reload fixture data
    loadData()->
    //And to start with clean session, the browser will be restarted
    restart()->

    //isAttribute() method will check the user attribute given
  ...
  //Day 18 Test AJAX
  //this method will set the http header to the next request on browser
  ...
  //Day 19 Internationalisation and localisation
